package io.github.luzzu.operations.signature;
...
import io.github.luzzu.operations.signature.GraphSigning;
...
public class GraphSigningTest extends Assert {
	...
	public void createAndValidateHash_ModifyTriplePosition() throws IOException{
		Dataset d = RDFDataMgr.loadDataset("ranking/dataset1.nq");
		// create
		String creationHash = this.createHash(d);
		...
		while (iter.hasNext()){
			...
			gs.addHash(ResourceCommons.statementToQuad(stmt, ModelFactory.createDefaultModel().createResource()));
		}
		...
		for(Statement stmt : stmts){
			gs.addHash(ResourceCommons.statementToQuad(stmt, ModelFactory.createDefaultModel().createResource("urn:a50f843c-1649-4f52-bda0-28b0ead913d2")));
		}
	...
	public void createAndValidateHash_ModifyTripleData() throws IOException{
		...
		for(Statement stmt : stmts){
			if ((stmt.getSubject().toString().contains("urn:obs3")) && (stmt.getPredicate().toString().equals(DAQ.value.toString()))){
				stmt = new StatementImpl(stmt.getSubject(), stmt.getPredicate(), ResourceCommons.generateDoubleTypeLiteral(1.0d));
			}
			gs.addHash(ResourceCommons.statementToQuad(stmt, ModelFactory.createDefaultModel().createResource("urn:a50f843c-1649-4f52-bda0-28b0ead913d2")));
		}
	...
	public void bigDataSet() throws IOException{
		...
		while(it.hasNext()){
			...
			while (iter.hasNext()){
				Statement stmt = iter.next();
				gs.addHash(ResourceCommons.statementToQuad(stmt, ModelFactory.createDefaultModel().createResource(nm)));
			}
		}
